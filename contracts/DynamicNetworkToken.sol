...
AccessControl
{
    ...
    uint256 public  totalWallets = 0;

    // Network Based Burn.

    uint256 networkBasedBurn = 1000000 ether;
    uint256 public nextBurn = 1;
    ...
    // Getter for currentSupply.

    function getCurrentSupply() public view returns(uint256){
        return(currentSupply/(10**18));
    }
    ...
    function calculateMint(uint256 amount) public returns(uint256){
        ...
        uint256 toBeMinted = SafeMath.add(prevAmount,amount);
        prevAmount = amount;
    ...
    function calculateBurn(uint256 amount) public returns(uint256){
        ...
        uint256 toBeBurned = SafeMath.add(prevAmount,amount);
        prevAmount = amount;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(totalWallets>=nextBurn && SafeMath.sub(currentSupply,amount)>=minimumSupply)
        {
        ...
        if(hasRole(LIQ_PROVIDER, msg.sender))
        {
            if(isExpMint)
            {
